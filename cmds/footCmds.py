...
class BipedFootPuppet(object):
    """this is the biped foot puppet class, which will be used to create a real biped foot"""
    ...
    def create_joint_setup(self, side = None, name = [None, None, None], position = [[0, 2, 0], [0, 0, 2], [0, 0, 1]]):
...
#        ikJoints   = jointChain.IkChain(side = side, name = name, position = position, mirror = True, ikSolver = 'ikRPsolver', radius = 0.75)
    ...
    def __create(self, side = None, name = [None, None, None], position = [[0, 2, 0], [0, 0, 2], [0, 0, 1]]):
        self.reload_modules()
