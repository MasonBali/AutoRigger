...
class Joint(object):
    ...
    def __init__(self, side = None, name = None, suffix = None, 
                 position = [0.0, 0.0, 0.0], parent = True, radius = 1):
        ########################################################################        
        #vars
        self.name     = []
        self.position = []
        ...
        self.__create(side = side, name = name, suffix = suffix, 
                      position = position, parent = parent, radius = radius)
    #end def __init__()
        ...
        if name == None:
            ...
            if side == None:
                ...
                if suffix == None:
                    ...
                    self.name = side + '_' + name + '_' + suffix 
    #end def check_joint_name()

    def create_joint(self, position = [0.0, 0.0, 0.0], parent = True, radius = 1):
        ########################################################################
        if parent == True:
            jnt = cmds.joint(name = self.name, position = position, 
                             radius = radius)
            if "|" in jnt:
        ...
        return self.name
    #end def create_joint()

    def __create(self, side = None, name = None, suffix = None, 
                 position = [0.0, 0.0, 0.0], parent = True, radius = 1):
        ########################################################################
